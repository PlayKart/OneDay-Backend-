...
app.get("/api/user", verifyUser, async (req, res) => {
  ...
  try {
    ...
    if (!user) {
      ...
      console.log("CREATING USER:", newUserPayload);
    ...
    console.error("GET USER CRITICAL ERROR:");
    console.error(error);
...
app.post("/api/complete", verifyUser, async (req, res) => {
  ...
  try {
    ...
    // TODAY COMPLETIONS
    // ==============================
    const { data: todayCompletions, error: todayError } =
      await supabase
        .from("completions")
        .select("id")
        .eq("userId", uid)
    ...
    console.error("COMPLETE HABIT ERROR:");
    console.error(error);
...
app.post("/api/chat", verifyUser, async (req, res) => {
  ...
  try {
    ...
    // STORE USER MESSAGE
    // ==============================
    const { error: insertUserMessageError } =
      await supabase
        .from("chats")
        .insert([{
          user_id: uid,
...
console.log("OPENAI RESPONSE:", aiData);
    ...
    // ==============================
    const { error: insertAssistantError } =
      await supabase
        .from("chats")
        .insert([{
          user_id: uid,
